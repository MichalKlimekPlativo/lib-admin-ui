module api.liveedit.text {
    ...
    export class TextComponentView extends ComponentView<TextComponent> {
        ...
        private doInitEditor() {
            ...
            this.htmlAreaEditor =
                new HTMLAreaBuilder().setEditorContainerId(this.getId() + '_editor').setAssetsUri(assetsUri).setInline(true).onCreateDialog(
                    event => {
                this.currentDialogConfig = event.getConfig();
                    ...
                    this.getContent()).setEditableSourceCode(this.editableSourceCode).setContentPath(
                    this.getContentPath()).setApplicationKeys(this.getApplicationKeys()).createEditor();
